...
=================================
Wraps AdvancedCollaborativeFiltering.recommend_similar_and_popular() in a
simple web UI: pick a movie, get back 5 recommendations that are both
similar in taste AND popular.

HOW TO RUN
----------
1. Put this file in a folder.
2. In that SAME folder, create a subfolder called "result/" and place these
   pkl files inside it:
       - ratings_dense.pkl       (DataFrame with columns: userId, movieId, rating)
       - user_item_matrix.pkl    (pivot table: index=userId, columns=movieId, values=rating)
       - movies.pkl  [OPTIONAL]  (DataFrame with columns: movieId, title)
         -> only needed so the UI can show movie TITLES instead of raw IDs.
            If your file has a different name/columns, edit MOVIES_FILE /
            MOVIE_ID_COL / TITLE_COL below.
3. Install dependencies:
       pip install gradio pandas scikit-learn
4. Run:
       python app.py
5. Gradio will print a local URL (and a public one if share=True) to open
   in your browser.
"""
